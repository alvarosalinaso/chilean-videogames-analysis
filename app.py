...
def load_data():
    ...
    for _ in range(220):
        year = np.random.choice(
            years,
            p=np.array([1, 1, 1, 1, 2, 3, 5, 8, 12, 14, 16, 14, 10, 8, 4], dtype=float)
            / 100,
        )
        ...
        price = (
            0
            if platform == "Itch.io" and np.random.rand() < 0.65
            else round(
                np.random.choice(
                    [0, 1.99, 4.99, 9.99, 14.99, 19.99],
                    p=[0.1, 0.1, 0.25, 0.3, 0.15, 0.1],
                ),
                2,
            )
        )
        reviews = (
            int(np.random.exponential(120))
            if platform == "Steam"
            else int(np.random.exponential(40))
        )
        score = round(min(10, max(1, np.random.normal(7.2, 1.5))), 1)
        dev_size = np.random.choice(
            ["Solo", "Mini (2-5)", "Pequeño (6-15)"], p=[0.55, 0.35, 0.10]
        )
        revenue_est = round(
            price
            * reviews
            * np.random.uniform(0.3, 0.7)
            * (1.5 if platform == "Steam" else 0.2),
            0,
        )
        launch_month = np.random.randint(1, 13)
        sentiment = round(
            min(1.0, max(-1.0, (score - 5) / 5 + np.random.normal(0, 0.15))), 2
        )
        games.append(
            {
...
with k2:
    ...
    st.metric(
        ...
        f"${rev / 1e6:.1f}M USD" if rev > 1e6 else f"${rev:,.0f}",
    )
with k3:
...
with tab1:
    ...
    for plat in sel_platforms:
        ...
        fig_trend.add_trace(
            go.Scatter(
                ...
                mode="lines+markers",
                name=plat,
                line={"color": colors_plat.get(plat, COLORS["accent"]), "width": 3},
                marker={"size": 8},
                fill="tozeroy",
    ...
    fig_trend.add_vline(
        ...
        annotation_text="📌 COVID-19: boom indie",
        ...
        for plat in sel_platforms:
            ...
            fig_rev.add_trace(
                go.Bar(
                    ...
                    y=d["revenue_est"],
                    name=plat,
                    marker_color=colors_plat.get(plat, COLORS["accent"]),
                    opacity=0.85,
                )
            )
    ...
    with col_b:
        ...
        heat_month = (
            df_f.groupby(["year", "launch_month"]).size().reset_index(name="count")
        )
        heat_pivot = heat_month.pivot(
            index="launch_month", columns="year", values="count"
        ).fillna(0)
        ...
        fig_hm.update_layout(
            **PLOTLY_THEME, title="Mapa de calor: lanzamientos por mes/año", height=320
        )
        fig_hm.update_xaxes(
            tickangle=-45, tickfont={"size": 10}, gridcolor="rgba(0,0,0,0)"
        )
        fig_hm.update_yaxes(tickfont={"size": 10}, gridcolor="rgba(0,0,0,0)")
...
with tab2:
    ...
    with col_l:
        ...
        fig_genre_bar.update_layout(
            **PLOTLY_THEME, title="Revenue promedio por género", height=380, bargap=0.25
        )
    ...
    genre_kpi_disp = genre_kpi.rename(
        columns={
            "genre": "Género",
            "count": "Juegos",
            "avg_rev": "Revenue prom. (USD)",
            "avg_score": "Score medio",
            "avg_sentiment": "Sentimiento",
        }
    ).round(2)
    st.dataframe(
        genre_kpi_disp.style.background_gradient(
            subset=["Revenue prom. (USD)"], cmap="YlGn"
        ),
    ...
    st.info(
        "💡 **Oportunidad detectada:** El género **Horror** tiene el mayor revenue promedio por juego en Steam (£320K muestra Tormented Souls). El nicho está subexplotado: solo 4 juegos chilenos de horror en Steam con >200 reviews."
    )
...
with tab3:
    ...
    with col_s1:
        ...
        fig_sent = px.histogram(
            ...
            labels={
                "sentiment": "Índice de sentimiento (-1 neg, +1 pos)",
                "count": "Juegos",
            },
        )
    ...
    with col_s2:
        ...
        fig_box.update_layout(
            **PLOTLY_THEME, height=320, legend={"orientation": "h", "y": -0.2}
        )
    ...
    fig_sent_rev.update_layout(
        **PLOTLY_THEME, height=380, legend={"orientation": "h", "y": -0.2}
    )
    ...
    st.warning(
        "🔬 **Metodología avanzada propuesta:** Implementar análisis de sentimientos real con BERT/RoBERTa sobre texto de reseñas (Steam Reviews API + Itch.io). El índice actual es un proxy score-based. Con NLP se puede detectar frustraciones específicas (bugs, dificultad, precio) para guiar el roadmap post-lanzamiento."
    )
...
with tab4:
    ...
    st.caption(
        "Cuadrante: Revenue potencial vs Saturación. Tamaño del círculo = score promedio."
    )
    ...
    opp["opportunity"] = opp["avg_rev"] / (
        opp["saturation"] + 0.1
    )  # high rev, low sat = opportunity
    ...
    fig_opp.update_xaxes(
        title="Saturación relativa (0=nicho, 1=masivo)", range=[-0.05, 1.1]
    )
